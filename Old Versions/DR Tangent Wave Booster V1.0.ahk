...
global TIMELINE_WINDOW_POS := {"var": "TIMELINE_WINDOW_POS", "name": "Position of the Timeline on Edit Tab", "posX": 2150, "posY": 1250}
global TIMELINE_TIMEBAR_POS := {"var": "TIMELINE_TIMEBAR_POS", "name": "Timeline Timebar on Edit Tab", "posX": 2050, "posY": 1225}
...
Gui Add, Button, x10 y10 w950 h30 gStartButton, Start DR Tangent Wave Booster

;Add each button for config each tools position on the screen
For index, object in _positionsArray {
    name := object.name
    varName := object.var
    Gui Add, Button, x%_counterButtonX% y%_counterButtonY% w150 h30 v%varName% gGenericButton, %name%
    
    _counterButtonY := _counterButtonY + 30
    if (index == 30){
        _counterButtonY := _startPositionButtonsY
        _counterButtonX := _counterButtonX + 160
    }Else if (index == 60 or index == 90 or index == 120 or index == 150){
        _counterButtonY := _startPositionButtonsY
        _counterButtonX := _counterButtonX + 160
    }
}

Gui Add, Button, x10 y980 w950 h30 gSaveFileButton, Save new positions
    
Gui Show, w970 h1020, Window
return
GuiEscape:
GuiClose:
    ExitApp
...
; == White Balance, togles, resets and more ==
...
^Numpad1:: ;Ctrl+Numpad1 - Select Preview Window (Edit Tab)
	MouseClick Left, PREVIEW_WINDOW_POS["posX"], PREVIEW_WINDOW_POS["posY"], 1, 1
return

^Numpad2:: ;Ctrl+Numpad2 - Select Timeline (Edit Tab)
	MouseClick Left, TIMELINE_WINDOW_POS["posX"], TIMELINE_WINDOW_POS["posY"], 1, 1
return

^Numpad3:: ;Ctrl+Numpad3 - Open/Close Media Pool (Edit Tab)
	MouseClick Left, MEDIA_POOL_BUTTON_POS["posX"], MEDIA_POOL_BUTTON_POS["posY"], 1, 1
    MouseClick Left, FIRST_CLIP_POOL_POS["posX"], FIRST_CLIP_POOL_POS["posY"], 1, 1
return

^Numpad4:: ;Ctrl+Numpad4 - Open/Close Effects Library (Edit Tab)
	MouseClick Left, EFFECTS_BUTTON_POS["posX"], EFFECTS_BUTTON_POS["posY"], 1, 1
return

^Numpad5:: ;Ctrl+Numpad5 - Open/Close Edit Index (Edit Tab)
	MouseClick Left, INDEX_BUTTON_POS["posX"], INDEX_BUTTON_POS["posY"], 1, 1
return

^Numpad6:: ;Ctrl+Numpad6 - Open/Close Sound Library (Edit Tab)
	MouseClick Left, SOUND_LIB_BUTTON_POS["posX"], SOUND_LIB_BUTTON_POS["posY"], 1, 1
    MouseClick Left, SEARCH_SOUND_POS["posX"], SEARCH_SOUND_POS["posY"], 1, 1
return

^Numpad7:: ;Ctrl+Numpad7 - Open/Close Mixer (Edit Tab)
	MouseClick Left, MIXER_BUTTON_POS["posX"], MIXER_BUTTON_POS["posY"], 1, 1
return

^Numpad8:: ;Ctrl+Numpad8 - Open/Close Metadata (Edit Tab)
	MouseClick Left, METADATA_BUTTON_POS["posX"], METADATA_BUTTON_POS["posY"], 1, 1
return

^Numpad9:: ;Ctrl+Numpad9 - Open/Close Inspector (Edit Tab)
	MouseClick Left, INSPECTOR_BUTTON_POS["posX"], INSPECTOR_BUTTON_POS["posY"], 1, 1
return
