...
;Set more than one thread per hot key so your hotkeys still working when inside a loop
...
;André Rodrigues Variables

;Save all the positions in this array
Global _positionsArray := []

;Counter to navigate inside the _positionsArray and get the actual object to be set
Global _positionCounter := 1

;Actual object being set
Global _actualObject

;Set a variable to know when Right Button is pressed
Global RButton_Pressed := False

;End of André Rodrigues Variables

{
	WM_ACTIVATE := 0x06
	WM_KILLFOCUS := 0x08
	WM_RBUTTONDOWN := 0x201
	WM_RBUTTONUP := 0x202
...
ShowMainGui:
{
    ...
    ;Start loading all the positions
    LoadPositions()
return
}

;===== André Rodrigues Code =====
...
DisplayArray(header, array) {
	...
	for index, var in array
		out .= "address: " var.address "`tclick: " var.click "`tvar: " var.var "`n"
	MsgBox, % out
}
...
CreateAddressFile(){
   For arrayNum, array in _positionsArray
        For index, varclickaddress in ["var", "click", "address"]
        IniWrite, % array[varclickaddress], %A_ScriptDir%\test.ini, POSITION_ADDRESS, %arrayNum%%varclickaddress%
...
LoadPositions(){
    ;Set a counter from 1 to 3 so loop knows if it is Var (1), address(2), click(3)
    counter := 1
    ...
    ;Loop through all_positions_names.ini file to populate the _positionArray(x) arrays
    Loop, read, %A_ScriptDir%\all_positions_names.ini
    {
        Loop, parse, A_LoopReadLine, %A_Tab%
        {
            positionName := A_Loopfield
            ...
            Loop, 3{
                switch (A_index){
                    Case 1:
                        ...
                        _positionsArray[arrayPos].var := A_Loopfield
                    continue
                    Case 2:
                        _positionsArray[arrayPos].click := False
                    continue
                    Case 3:
                        _positionsArray[arrayPos].address := ""
    ...
    ;DisplayArray("Positions Array", _positionsArray)
    ;arrayLenght := NumGet(&_positionsArray + 4*A_PtrSize)
    ;MsgBox %arrayLenght%
}

getPositionObject(position){
    Return _positionsArray[position]
}
...
StartSearch(){
    _actualObject := getPositionObject(_positionCounter)
    
    if(_actualObject != ""){
        posName := _actualObject.var
    
        ToolTip, %posName%
        
        SetTimer, ShowTooltipOnMouse, 50
...
GetItem(shouldClick := False, needID := 0){
    Gui Main: Default
	if Not CH {
	...
	if Not DllCall("IsWindowVisible", "Ptr",Win.Acc) {
		...
		PostMessage, %WM_RBUTTONDOWN%, , , SysTreeView321, % "ahk_id" Win.Acc
	}
    
    For index, object in _positionsArray
    if (object.var == _actualObject.var){
        Switch (needID){
            Case 0:
                object.address := "0\" . GetAccPath(Acc).path
            Case 1:
                object.address := "1\" . GetAccPath(Acc).path
            Case 2:
                object.address := "2\" . GetAccPath(Acc).path
        }
...
ShowTooltipOnMouse(){
    MouseGetPos x, y
    posName := _actualObject.var
    Tooltip, %posName%, x + 20, y + 20
}

;===== End of André Rodrigues Code =====
