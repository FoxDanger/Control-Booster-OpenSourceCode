...
;Set the actual mode. Can be: "EDIT", "COLOR", 
global _actualMode := "EDIT"
...
;Save the actual status of Inspector Window. True is open, false is close.
global _inspectorStatus := false

;Save the actual status of Media Pool Window. True is open, false is close.
global _mediaPoolStatus := true
...
_startOSC2AHK(){
    ...
    OnMessage(0x1001, "_enterEditMode")

    DllCall("OSC2AHK.dll\addListener", AStr, "/enterColorMode", UInt, 0x1002, UInt, oscTypeInt)
    OnMessage(0x1002, "_enterColorMode")
    
    DllCall("OSC2AHK.dll\addListener", AStr, "/openCloseInspector", UInt, 0x1003, UInt, oscTypeInt)
    OnMessage(0x1003, "_openCloseInspector")
}
...
;Set the mode to COLOR, open color page and set the panel to Primaries Wheels with custom curves (default davinci mode)
...
_openCloseInspector(oscType, data, msgID, hwnd){
    ...
    if (_inspectorStatus == false){
        _inspectorStatus := true
    }Else{
        _inspectorStatus := false
    }
    
    object := _returnObject("pos_edit_top_bts_inspector")
...
;Change the mode from EDIT to COLOR
_changeMode(mode){
    Switch (mode){
        Case "EDIT":
            _actualMode := "EDIT"
            ...
            if(_inspectorStatus == false){
                _openCloseInspector(oscTypeInt, 0, 0, hWnd)                
            }
