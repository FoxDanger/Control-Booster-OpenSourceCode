...
Gui Add, Tab3, x8 y176 w1905 h900 Choose1, Edit Page Interface|Inspector Transform|Color Page Interface|Wheels/Jogs|Primaries Tools|Primaries YRBG|Log RGB|Log Tools|HDR Controllers|HDR Tools|HDR Zone|RGB Mixer|Curves UI|Custom Curves|Hue Curves|Qualifier|Qualifier Matte Finess|Power Windows|BSM|Key|Motion Effects|Scopes|Other
...
Gui Add, Button, g_set_variable_button vpos_primaries_first_wheel_reset x528 y848 w150 h50, First Wheel Reset
Gui Add, Button, g_set_variable_button vpos_primaries_first_wheel x528 y912 w150 h50, First Wheel
Gui Add, Button, g_set_variable_button vpos_primaries_first_jog x528 y976 w150 h50, First Jog
Gui Add, Button, g_set_variable_button vpos_primaries_second_wheel_reset x768 y848 w150 h50, Second Wheel Reset
Gui Add, Button, g_set_variable_button vpos_primaries_second_wheel x768 y912 w150 h50, Second Wheel
Gui Add, Button, g_set_variable_button vpos_primaries_second_jog x768 y976 w150 h50, Second Jog
Gui Add, Button, g_set_variable_button vpos_primaries_third_wheel_reset x1000 y848 w150 h50, Third Wheel Reset
Gui Add, Button, g_set_variable_button vpos_primaries_third_wheel x1000 y912 w150 h50, Third Wheel
Gui Add, Button, g_set_variable_button vpos_primaries_third_jog x1000 y976 w150 h50, Third Jog
Gui Add, Button, g_set_variable_button vpos_primaries_fourth_wheel_reset x1240 y848 w150 h50, Fourth Wheel Reset
Gui Add, Button, g_set_variable_button vpos_primaries_fourth_wheel x1240 y912 w150 h50, Fourth Wheel
Gui Add, Button, g_set_variable_button vpos_primaries_fourth_jog x1240 y976 w150 h50, Fourth Jog
...
Gui Add, Button, g_set_variable_button vpos_primaries_black_point_picker x328 y544 w150 h50, Black Point Picker
Gui Add, Button, g_set_variable_button vpos_primaries_white_point_picker x1448 y488 w150 h50, White Point Picker
...
Gui Tab, 6
Gui Add, Picture, x480 y232 w956 h406 +0x400000 , %A_ScriptDir%\images\Primaries Bars Panel.png
...
Gui Tab, 17
Gui Add, Picture, x672 y320 w570 h400, F:\André Rodrigues\Media Environment\Produtos\TWB 2 - Develop\GIT\develop\images\Qualifier Matte Finesse Panel.png
...
;Start load all positions and populate _positionsArrays(x) (this function NEED to be called inside the GUI construction at the end of it)
LoadPositions()
...
StartOSC2AHK(){
    ...
    if (success != 0){
        msgbox, Failed to open port for OSC2AHK! Please check your firewall and free the port 7002 and/or add an exception on your anti-virus for this DLL and TWB 2.
    }Else{
...
LoadPositions(){
    ;Reset _positionsArray(x)
    _positionsArray1 := []
    _positionsArray2 := []
    _positionsArray3 := []

    ;Set a counter so loop knows if it is var or adress
    counter := 1
    ...
    ;Loop through objects_positions.ini file to populate the arrays _positionsArray(x)
    Loop, read, %A_ScriptDir%\objects_positions.ini
    {
        Loop, parse, A_LoopReadLine, %A_Tab%
        {
            if (InStr(A_Loopfield, _davinciLayout) == 0){
                continue
            }
            ...
            if (InStr(A_Loopfield, "Array1") != 0){
                switch (counter){
                    ...
                    Case 2:
                        ...
                        _positionsArray1[arrayPos].adress := SubStr(A_Loopfield, startingPos + 1)
                ...
                switch (counter){
                    ...
                    Case 2:
                        ...
                        _positionsArray2[arrayPos].adress := SubStr(A_Loopfield, startingPos + 1)
                        
                        counter := 1
                        arrayPos++
                    continue
                }
            }Else if (InStr(A_Loopfield, "Array3") != 0){
                ;If to reset the variables so array 3 can start from the begin
                if (arrayCicle == 2){
                    arrayPos := 1
                    counter := 1
                    arrayCicle := 3
                ...
                switch (counter){
                    Case 1:
                        object := []
                        _positionsArray3.Push(object)
                        
                        startingPos := InStr(A_Loopfield, "=")
                        
                        _positionsArray3[arrayPos].var := SubStr(A_Loopfield, startingPos + 1)
                    ...
                    Case 2:
                        ...
                        _positionsArray3[arrayPos].adress := SubStr(A_Loopfield, startingPos + 1)
    ...
    ;DisplayArray("Positions Array 1", _positionsArray1)
    ;DisplayArray("Positions Array 2", _positionsArray2)
    ;DisplayArray("Positions Array 3", _positionsArray3)
}

;Load the Edit Windows status of the last session from windows_status.ini
LoadWindowsStatus(){
    ;Set a counter so loop knows if it is var or status
    counter := 1
    ...
    ;Loop through windows_status.ini file to populate the array _editPageWindowsStatus
...
;Load the Edit Windows status of the last session from windows_status.ini
LoadFindTextHashs(){
    ;Set a counter so loop knows if it is var or text
    counter := 1
    
    ;Set a counter for the Array Position
    arrayPos := 1
    
    ;Loop through custom_curves_hashs.ini file to populate the array _dotsHashsArray
...
;Get the mouse actual position and store the X and Y on the respective object variable inside the _positionsArray(x)
ChangePositionVariable(){
    ...
    if (variableFound == "false"){
        For index, object in _positionsArray3
        if (object.var == _variableToSet) {
            object.x := mousePosX
            object.y := mousePosY
...
_accAction(path, child){
    ;MsgBox %path%
    coordmode,mouse,screen
    WinGet, hWnd, ID, A
    oAcc := Acc_Get("Object", path, child, "ahk_id " hWnd)
    action = % oAcc.accDefaultAction(0)
    return action
}

_accClick(objectName, child) 
{
    ReleaseAllControls()
    ...
    coordmode,mouse,screen
    WinGet, hWnd, ID, A
    oAcc := Acc_Get("Object", object.adress, child, "ahk_id " hWnd)
    oRect := Acc_Location(oAcc, 0)
    
    MouseClick Left, oRect.x + (oRect.w/2) , oRect.y + (oRect.h/2), 1, 0
}

_accClickDefault(objectName, child) 
{
	ReleaseAllControls()	
...
_accGetPosition(objectName, child){
    object := GetObjectOnPositionsArray(objectName)
    ...
    oRect := Acc_Location(oAcc, 0)
    position := {}
    position.x := oRect.x + (oRect.w/2)
    position.y := oRect.y + (oRect.h/2)
...
MoveMouseToPosition(objectName, child := "0"){
    position := _accGetPosition(objectName, child)
    
    MouseMove, position.x, position.y, 0
}

;Move mouse and click on object
MoveMouseAndClick(objectName, child := "0", addRelativeX := 0, addRelativeY := 0){
    position := _accGetPosition(objectName, child)
    
    MouseClick Left, position.x + addRelativeX, position.y + addRelativeY, 1, 0
}

;Move mouse and double click on object
MoveMouseAndDoubleClick(objectName, child := "0"){
    position := _accGetPosition(objectName, child)
    
    MouseClick Left, position.x, position.y, 2, 10
}
...
MoveMouseAndDrag(data, axis, objectName := "NULL", isDot := False){
    if (_activeController != objectName){
        ...
        if (objectName != "NULL"){
            position := _accGetPosition(objectName, "0")
...
_motionEffectsSpatialRadius(oscType, data, msgID, hwnd){
    ChangeColorPanel("MOTIONEFFECTS")
...
_keyReset(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "KEY") == False)
    Return
...
_qualifierResetLuminanceKnob(oscType, data, msgID, hwnd){
    if (data < 0){
        ...
        CheckIfColorPanel("QUALIFIERRGB")
...
_scopesLowPassFilter(oscType, data, msgID, hwnd){
    if (CheckIfScopesAndData0(data) == False)
    Return
...
_pwAddNodeCurve(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "PW") == False)
    Return
...
_bsmReset(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "BSM") == False)
    Return
...
_rgbMixerReset(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "RGBMIXER") == False)
    Return
...
_hueCurvesSatXLum(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "HUECURVES") == False)
    Return
...
_customCurvesCopyToAll(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "CUSTOMCURVES") == False)
    Return
...
_customCurvesResetKnob(oscType, data, msgID, hwnd){
    if (CheckIfColorPanel(data, "CUSTOMCURVES") == False)
    Return
...
_logResetAll(oscType, data, msgID, hwnd){
    ...
    MoveMouseAndClick("pos_log_reset")
}
...
_barsResetAll(oscType, data, msgID, hwnd){
    ...
    MoveMouseAndClick("pos_bars_reset")
}
...
_barsLumMix(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_lum_mix", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_lum_mix")
        Return
    }
}
...
_barsMidDetail(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_middetail", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_middetail")
        Return
    }
}
...
_barsHue(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_hue", 50)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_hue")
        Return
    }
}
...
_barsSaturation(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_saturation", 50)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_saturation")
        Return
    }
}
...
_barsHighlight(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_high_light", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_high_light")
        Return
    }
}
...
_barsShadow(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_shadow", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_shadow")
        Return
    }
}
...
_barsCoolBoost(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_cool_boost", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_cool_boost")
        Return
    }
}
...
_barsPivot(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_pivot", 0.435)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_pivot")
        Return
    }
}
...
_barsContrast(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_contrast", 1)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_contrast")
        Return
    }
}
...
_barsTint(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_tint", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_tint")
        Return
    }
}
...
_barsTemperature(oscType, data, msgID, hwnd){
    ...
    if (data == 0){
        ResetForSpecificValue("pos_bars_temp", 0)
        Return
    }Else{
        MoveMouseAndDrag(data, "x", "pos_bars_temp")
        Return
    }
}
...
;Button - Open the Vide Inspector Window
...
;Button - Open the Audio Inspector Window
...
GetObjectOnPositionsArray(objectName){
    ...
    if (object.var == objectName) {
        object.state := _accAction(object.adress, "0")
        
        if (object.state == "Check"){
            object.state := True
        }Else{
            object.state := False
        }
...
ChangeMode(mode){
    ...
    Switch (mode){
        ...
        Case "COLOR WHEELS":
            ...
            _accClick("pos_color_page_bts_color_wheels", "0")
        Return
        Case "HDR":
            _actualMode := "HDR"
            _accClick("pos_color_page_bts_hdr_wheels", "0")
        Return
        Case "CURVES":
            _actualMode := "CURVES"
            _accClick("pos_color_page_bts_curves", "0")
        Return
        Case "RGBMIXER":
            _actualMode := "RGBMIXER"
            _accClick("pos_color_page_bts_rgb_mixer", "0")
        Return
        Case "BSM":
            _actualMode := "BSM"
            _accClick("pos_color_page_bts_bsm", "0")
        Return
        Case "PW":
            _actualMode := "PW"
            _accClick("pos_color_page_bts_power_windows", "0")
        Return
        Case "QUALIFIER":
            _actualMode := "QUALIFIER"
            _accClick("pos_color_page_bts_qualifier", "0")
        Return
        Case "KEY":
            _actualMode := "KEY"
            _accClick("pos_color_page_bts_key", "0")
        Return
        Case "MOTIONEFFECTS":
            _actualMode := "MOTIONEFFECTS"
            _accClick("pos_color_page_bts_motion_effects", "0")
        Return
    }
}
...
ChangeColorPanel(panel){
    ...
    Switch (panel){
        Case "PRIMARIES":
            ...
            _accClick("pos_primaries_wheels_dot", "0")
        Return
        Case "BARS":
            if (_actualMode != "COLOR WHEELS"){
                ChangeMode("COLOR WHEELS")                
            ...
            _accClick("pos_primaries_bars_dot", "0")
        Return
        Case "LOG":
            if (_actualMode != "COLOR WHEELS"){
                ChangeMode("COLOR WHEELS")                
            ...
            _accClick("pos_primaries_log_dot", "0")
        Return
        Case "HDR":
            if (_actualMode != "HDR"){
                ChangeMode("HDR")            
        ...
        Case "CUSTOMCURVES":
            ...
            _accClick("pos_curves_buttons_custom_dot", "0")
        Return
        Case "HUECURVES":
            if (_actualMode != "CURVES"){
                ChangeMode("CURVES")
            ...
            _accClick("pos_curves_buttons_huexhue_dot", "0")
        Return
        Case "RGBMIXER":
        ...
        Case "QUALIFIERHSL":
            ...
            _accClick("pos_qualifier_hsl_dot", "0")
        Return
        Case "QUALIFIERRGB":
            if (_actualMode != "QUALIFIER"){
                ChangeMode("QUALIFIER")
            ...
            _accClick("pos_qualifier_rgb_dot", "0")
        Return
        Case "KEY":
            if (_actualMode != "KEY"){
                ChangeMode("KEY")
            ...
            _actualColorPanel := "KEY"
        Return
        Case "MOTIONEFFECTS":
            if (_actualMode != "MOTIONEFFECTS"){
                ChangeMode("MOTIONEFFECTS")
            }
            
            _actualColorPanel := "MOTIONEFFECTS"
        Return
    }
}
...
ChangeHueCurvesPanel(panel){
    ...
    Switch (panel){
        ...
        Case "HUEXSAT":
            ...
            _accClick("pos_curves_buttons_huexsat_dot", "0")
        Return
        Case "HUEXLUM":
            _actualHueCurvesPanel := "HUEXLUM"
            
            _accClick("pos_curves_buttons_huexlum_dot", "0")
        Return
        Case "LUMXSAT":
            _actualHueCurvesPanel := "LUMXSAT"
            
            _accClick("pos_curves_buttons_lumxsat_dot", "0")
        Return
        Case "SATXSAT":
            _actualHueCurvesPanel := "SATXSAT"
            
            _accClick("pos_curves_buttons_satxsat_dot", "0")
        Return
        Case "SATXLUM":
            _actualHueCurvesPanel := "SATXLUM"
            
            _accClick("pos_curves_buttons_satxlum_dot", "0")
        Return
    }
}
...
ChangeBSMPanel(panel){
    ...
    Switch (panel){
        Case "BLUR":
            ...
            _accClick("pos_bsm_blur_dot", "0")
        Return
        Case "SHARPEN":
            _actualBSMPanel := "SHARPEN"
            
            _accClick("pos_bsm_sharpen_dot", "0")
        Return
        Case "MIST":
            _actualBSMPanel := "MIST"
            
            _accClick("pos_bsm_mist_dot", "0")
        Return
    }
}
...
ChangeVideoAudioInspectorPanel(panel){
    if (panel == 0){
        ...
        _accClick("pos_inspector_audio", "0")
        
        Return
    }Else{
        GetWindow("pos_inspector_video").status := 1
        
        _accClick("pos_inspector_video", "0")
...
ChangeMatteFinessePage(page){
    ...
    if (page == 1){
        _accClick("pos_matte_finesse_1", "0")
        
        SetWindowStatus(object.var, 0)
    }Else{
        _accClick("pos_matte_finesse_2", "0")
...
;Check if the scopes are closed or opened. If they are closed, it will open it.
CheckIfScopesAndData0(data){
    if (data == 0)
    Return False
    ...
    _accClick("pos_color_page_bts_scopes", "0")
...
CheckIfColorPanel(panel, openHDRZone := False){
    ChangeColorPanel(panel)
...
;Check if _actualPage is EDIT, if it is Returns True. If it isnt on EDIT mode, change page to EDIT and return False.
CheckIfEdit(){
    if (_actualPage != "EDIT"){
        ChangePage("EDIT")
...
;Check if _actualPage is EDIT, if it is, open the inspector window if closed and open video or audio panel (based on videoOrAudio parameter), after that return true. If it isnt on EDIT mode, change page to EDIT and return False.
...
ResetForSpecificValue(objectName, value){
    MoveMouseAndClick(objectName)
    Sleep 10
    Send %value%
    Sleep 10
    Send {Enter}
...
ResetHDRPanels(reset){
    Switch (reset){
        Case "ALL":
            MoveMouseAndClick("pos_hdr_dots_menu", "0")
            sleep 50
            MoveMouseAndClick("pos_hdr_dots_menu", "0", 50, 50)
            Sleep 50
            MoveMouseAndClick("pos_hdr_dots_menu", "0", 400, 50)
        Return
        Case "COLOR":
            _accClickDefault("pos_hdr_reset_color", "0")
            MsgBox "AQUI"
        Return
        Case "ZONE":
            _accClickDefault("pos_hdr_reset_zone", "0")
        Return
    }
}
...
DisplayArray(header, array) {
    ...
    for index, var in array
        out .= "adress: " var.adress "`tvar: " var.var "`n"
    MsgBox, % out
}
...
_save_positions_bt(){
   ;UpdateIniResScaleFile(_davinciLayout)
}
...
;UpdateIniResScaleFile(resolutionScale) {
    ;For arrayNum, array in _positionsArray1
        ...
        ;IniWrite, % array[varxy], %A_ScriptDir%\object_positions.ini, %resolutionScale%, Array1 - %_davinciLayout% - %arrayNum%%varxy%
        ;
    ;For arrayNum, array in _positionsArray2
        ;For index, varxy in ["var", "x", "y"]
        ;IniWrite, % array[varxy], %A_ScriptDir%\object_positions.ini, %resolutionScale%, Array2 - %_davinciLayout% - %arrayNum%%varxy%
;}


;=== Change values when change combobox Itens ===

;Change the combobox for the latest resolution/scale saved in the _davinciLayout variable and write it on the INI file. After that, load all the positions inside the _positionsArray1 and _positionsArray2
;_change_resolution_and_scale(CtrlHwnd, GuiEvent, EventInfo, ErrLevel := "") {
    ;CoordMode, Mouse, Screen

    ;GuiControlGet, comboboxResolutionScale
    ;_davinciLayout := comboboxResolutionScale
    ;
    ;IniWrite, %_davinciLayout%, %A_ScriptDir%\resolution.ini, RESOLUTION, lastResScale
    ;
    ;LoadPositions()
    ;
    ;LoadFindTextHashs()
...
;=== Adding new itens for a combobox ===
...
;AddcomboboxItems(item) {
    ...
    ;GuiControl, Text, comboboxResolutionScale, %_davinciLayout%
...
;=== Calling a input box to add combobox values or do some other action ===
...
; /\ END OF MAYBE USEFUL CODE IN THE FUTURE /\