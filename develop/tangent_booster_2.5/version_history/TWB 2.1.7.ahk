...
;2.1 - Open the custom_curves_hash.ini and create a new field with the same name of your resolution/dpi Scale scheme (just copy/paste one that is already in the document and change the name of the resolution inside the []);
...
;TODO: There's two positions on array of positions that can be used for a new thing if necessary: pos_timeline (Array 2 - 185) and pos_preview_window (array 2 - 187).
...
;Create a variable for the resolution and scale combo box and set the actual PC Resolution and DPIScale - String - Can be: "FHD100/125", "4K100/125", "4K150/175/200" or any custom resoltuion/dpi scale created by the user.
...
;Set the actual Matte Finesse Shape on Qualifier Panel. Can be: "1" (Square), "2" (Circle - Default) and "3" (Diamond)
...
Gui Add, Picture, x740 y592 w440 h65, %A_ScriptDir%\images\Color Interface Tools.png
...
Gui Show, w1920 h1080, TWB 2.1.7
...
Osc2ahkAddOrRemoveListeners(addOrRemove := "add"){
    if (addOrRemove == "add"){
        ...
        OnMessage(0x1446, "_matteFinesseDenoise")
        ...
        ;Qualifier Listeners
        DllCall("OSC2AHK.dll\removeListener", AStr, "/matteFinesseDenoise", UInt, 0x1446, UInt, oscTypeInt)
        OnMessage(0x1446, "_matteFinesseDenoise")
        ...
        DllCall("OSC2AHK.dll\removeListener", AStr, "/matteFinesseMode", UInt, 0x1441, UInt, oscTypeInt)
        OnMessage(0x1441, "_matteFinesseMode")
        
        DllCall("OSC2AHK.dll\removeListener", AStr, "/matteFinesseShape", UInt, 0x1440, UInt, oscTypeInt)
        OnMessage(0x1440, "_matteFinesseShape")
        
        DllCall("OSC2AHK.dll\removeListener", AStr, "/matteFinesseRadius", UInt, 0x1439, UInt, oscTypeInt)
        OnMessage(0x1439, "_matteFinesseRadius")
        
        DllCall("OSC2AHK.dll\removeListener", AStr, "/matteFinesseIterations", UInt, 0x1438, UInt, oscTypeInt)
        OnMessage(0x1438, "_matteFinesseIterations")
...
LoadPositions(){
    ...
    if !(resolutionFound){
        Loop, read, %A_ScriptDir%\default_ini_files\res_dpi_scale_variables.ini
        {
            Loop, parse, A_LoopReadLine, %A_Tab%
            {
                if (InStr(A_Loopfield, "Array1") != 0){
...
_motionEffectsSpatialRadius(oscType, data, msgID, hwnd){
    CheckIfColorPanel("MOTIONEFFECTS")
...
_motionEffectsReset(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "MOTIONEFFECTS") == False)
    Return
...
_logColorPicker(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "LOG") == False)
    Return
...
_barsWhitePointPicker(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "BARS") == False)
    Return
...
_primariesWhitePointPicker(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "PRIMARIES") == False)
    Return
...
_hdrZoneFalloff(oscType, data, msgID, hwnd){
    if (CheckIfColorPanelData0(data, "HDR", true) == False)
    Return
