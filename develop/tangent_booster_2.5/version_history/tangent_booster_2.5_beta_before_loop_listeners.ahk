...
;Version: 2.5 Beta
;You can find video tutorials about how to setup and use all the functions at: https://souandrerodrigues.com.br/tb
...
;- New Tracker Mode.
;- Implemented all the listeners and functions for Tracker panel.
;- New Sizing Modes.
;- Implemented all the listeners and functions for all the Sizing panels (Edit, Input, Output, Node and Reference).
;TODO:
...
;- Tangent Booster screen now resize with different DPI/Resolution Scales.
;- Added native support for more resolutions.
;- Implemented a new tool (BETA) to auto-setup the positions of the elements on the screen. Now you only need setup manually just a few things and there's a auto-guided method for this. You don't need goes and back to Tangent Booster and press the buttons to setup each position.
...
;- A new file for Custom Curves Hash (custom_curves_hashs.ini). Now it only has 3 types of hashs and the use don't need add more hashs when creating different types of resolutions/scale. Just need choose on Tangent Booster between the 3 modes.
...
;- In any mode you can use Alternate + F9 Key to start/stop Tangent Booster, so you can go out Davinci Screen without the controller click or move the mouse when you don't want that.
...
;6 - Create each OSC listener (add and remove listeners) for each new controller on Tangent Booster.AHK
;7 - Create each function for each listener on Tangent Booster.AHK
...
;Non-default Davinci Resolve shortcuts used (you need assign these shortcuts on your Davinci - You can use the Davinci Resolve Shortcut Map that comes with Tangent Booster):
...
; == Array for the position variables - Set the position of each element on the Davinci Resolve workarea (if your resolution and/or scale doesn't work with any of the presets, you need setup each position manualy using Tangent Booster tool) ==
...
Gui Add, Button, g_start_stop_bt x192 y8 w150 h50, Stop Tangent Booster
...
Gui Show, % "w" . 1920 . " h" . 1080, Tangent Booster 2.5 Beta
...
StartOSC2AHK(){
    ...
    if (success != 0){
        msgbox, Failed to open port for OSC2AHK! Please check your firewall and free the port %OSC_RECEIVE_PORT% and/or add an exception on your anti-virus for this DLL and Tangent Booster.
    }Else{
        msgbox, Tangent Booster is ready to be used!
    }
...
Osc2ahkAddOrRemoveListeners(addOrRemove := "add"){
    if (addOrRemove == "add"){
        ...
        DllCall("OSC2AHK.dll\addListener", AStr, "/tracking", UInt, 0x1659, UInt, oscTypeInt)
        OnMessage(0x1659, "_tracking")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingToggleClipFrame", UInt, 0x1660, UInt, oscTypeInt)
        OnMessage(0x1660, "_trackingToggleClipFrame")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingToggleWindowFX", UInt, 0x1661, UInt, oscTypeInt)
        OnMessage(0x1661, "_trackingToggleWindowFX")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingStepFramesTrack", UInt, 0x1662, UInt, oscTypeInt)
        OnMessage(0x1662, "_trackingStepFramesTrack")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingTrackReverse", UInt, 0x1663, UInt, oscTypeInt)
        OnMessage(0x1663, "_trackingTrackReverse")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingStopTracking", UInt, 0x1664, UInt, oscTypeInt)
        OnMessage(0x1664, "_trackingStopTracking")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingTrackForward", UInt, 0x1665, UInt, oscTypeInt)
        OnMessage(0x1665, "_trackingTrackForward")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingTrackReverseForward", UInt, 0x1666, UInt, oscTypeInt)
        OnMessage(0x1666, "_trackingTrackReverseForward")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingAddPoint", UInt, 0x1667, UInt, oscTypeInt)
        OnMessage(0x1667, "_trackingAddPoint")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingRemovePoint", UInt, 0x1668, UInt, oscTypeInt)
        OnMessage(0x1668, "_trackingRemovePoint")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingPreviousKeyframe", UInt, 0x1669, UInt, oscTypeInt)
        OnMessage(0x1669, "_trackingPreviousKeyframe")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingAddRemoveKeyframe", UInt, 0x1670, UInt, oscTypeInt)
        OnMessage(0x1670, "_trackingAddRemoveKeyframe")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingNextKeyframe", UInt, 0x1671, UInt, oscTypeInt)
        OnMessage(0x1671, "_trackingNextKeyframe")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingPan", UInt, 0x1672, UInt, oscTypeInt)
        OnMessage(0x1672, "_trackingPan")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingTilt", UInt, 0x1673, UInt, oscTypeInt)
        OnMessage(0x1673, "_trackingTilt")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingZoom", UInt, 0x1674, UInt, oscTypeInt)
        OnMessage(0x1674, "_trackingZoom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingRotate", UInt, 0x1675, UInt, oscTypeInt)
        OnMessage(0x1675, "_trackingRotate")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/tracking3D", UInt, 0x1676, UInt, oscTypeInt)
        OnMessage(0x1676, "_tracking3D")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingShowTracking", UInt, 0x1677, UInt, oscTypeInt)
        OnMessage(0x1677, "_trackingShowTracking")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingCopyData", UInt, 0x1678, UInt, oscTypeInt)
        OnMessage(0x1678, "_trackingCopyData")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingPasteData", UInt, 0x1679, UInt, oscTypeInt)
        OnMessage(0x1679, "_trackingPasteData")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/trackingResetAll", UInt, 0x1680, UInt, oscTypeInt)
        OnMessage(0x1680, "_trackingResetAll")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingModeInput", UInt, 0x1681, UInt, oscTypeInt)
        OnMessage(0x1681, "_sizingModeInput")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingModeEdit", UInt, 0x1682, UInt, oscTypeInt)
        OnMessage(0x1682, "_sizingModeEdit")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingModeOutput", UInt, 0x1683, UInt, oscTypeInt)
        OnMessage(0x1683, "_sizingModeOutput")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingModeNode", UInt, 0x1684, UInt, oscTypeInt)
        OnMessage(0x1684, "_sizingModeNode")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingModeReference", UInt, 0x1685, UInt, oscTypeInt)
        OnMessage(0x1685, "_sizingModeReference")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingResetAll", UInt, 0x1686, UInt, oscTypeInt)
        OnMessage(0x1686, "_sizingResetAll")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditPan", UInt, 0x1687, UInt, oscTypeInt)
        OnMessage(0x1687, "_sizingEditPan")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditTilt", UInt, 0x1688, UInt, oscTypeInt)
        OnMessage(0x1688, "_sizingEditTilt")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditRotate", UInt, 0x1689, UInt, oscTypeInt)
        OnMessage(0x1689, "_sizingEditRotate")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditLensCorrection", UInt, 0x1690, UInt, oscTypeInt)
        OnMessage(0x1690, "_sizingEditLensCorrection")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditDistortion", UInt, 0x1691, UInt, oscTypeInt)
        OnMessage(0x1691, "_sizingEditDistortion")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditZoomX", UInt, 0x1692, UInt, oscTypeInt)
        OnMessage(0x1692, "_sizingEditZoomX")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditZoomY", UInt, 0x1693, UInt, oscTypeInt)
        OnMessage(0x1693, "_sizingEditZoomY")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditLinkZoom", UInt, 0x1694, UInt, oscTypeInt)
        OnMessage(0x1694, "_sizingEditLinkZoom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditPitch", UInt, 0x1695, UInt, oscTypeInt)
        OnMessage(0x1695, "_sizingEditPitch")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditYaw", UInt, 0x1696, UInt, oscTypeInt)
        OnMessage(0x1696, "_sizingEditYaw")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditFlip", UInt, 0x1697, UInt, oscTypeInt)
        OnMessage(0x1697, "_sizingEditFlip")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditCropLeft", UInt, 0x1698, UInt, oscTypeInt)
        OnMessage(0x1698, "_sizingEditCropLeft")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditCropRight", UInt, 0x1699, UInt, oscTypeInt)
        OnMessage(0x1699, "_sizingEditCropRight")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditCropTop", UInt, 0x1700, UInt, oscTypeInt)
        OnMessage(0x1700, "_sizingEditCropTop")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditCropBottom", UInt, 0x1701, UInt, oscTypeInt)
        OnMessage(0x1701, "_sizingEditCropBottom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditSoftness", UInt, 0x1702, UInt, oscTypeInt)
        OnMessage(0x1702, "_sizingEditSoftness")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditResetSizing", UInt, 0x1703, UInt, oscTypeInt)
        OnMessage(0x1703, "_sizingEditResetSizing")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingEditResetCrop", UInt, 0x1704, UInt, oscTypeInt)
        OnMessage(0x1704, "_sizingEditResetCrop")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputPan", UInt, 0x1705, UInt, oscTypeInt)
        OnMessage(0x1705, "_sizingInputPan")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputTilt", UInt, 0x1706, UInt, oscTypeInt)
        OnMessage(0x1706, "_sizingInputTilt")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputZoom", UInt, 0x1707, UInt, oscTypeInt)
        OnMessage(0x1707, "_sizingInputZoom")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputRotate", UInt, 0x1708, UInt, oscTypeInt)
        OnMessage(0x1708, "_sizingInputRotate")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputWidth", UInt, 0x1709, UInt, oscTypeInt)
        OnMessage(0x1709, "_sizingInputWidth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputHeight", UInt, 0x1710, UInt, oscTypeInt)
        OnMessage(0x1710, "_sizingInputHeight")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputPitch", UInt, 0x1711, UInt, oscTypeInt)
        OnMessage(0x1711, "_sizingInputPitch")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputYaw", UInt, 0x1712, UInt, oscTypeInt)
        OnMessage(0x1712, "_sizingInputYaw")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputFlip", UInt, 0x1713, UInt, oscTypeInt)
        OnMessage(0x1713, "_sizingInputFlip")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputBlankingTop", UInt, 0x1714, UInt, oscTypeInt)
        OnMessage(0x1714, "_sizingInputBlankingTop")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputBlankingRight", UInt, 0x1715, UInt, oscTypeInt)
        OnMessage(0x1715, "_sizingInputBlankingRight")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputBlankingBottom", UInt, 0x1716, UInt, oscTypeInt)
        OnMessage(0x1716, "_sizingInputBlankingBottom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputBlankingLeft", UInt, 0x1717, UInt, oscTypeInt)
        OnMessage(0x1717, "_sizingInputBlankingLeft")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputSmooth", UInt, 0x1718, UInt, oscTypeInt)
        OnMessage(0x1718, "_sizingInputSmooth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputResetSizing", UInt, 0x1719, UInt, oscTypeInt)
        OnMessage(0x1719, "_sizingInputResetSizing")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingInputResetBlanking", UInt, 0x1720, UInt, oscTypeInt)
        OnMessage(0x1720, "_sizingInputResetBlanking")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputPan", UInt, 0x1721, UInt, oscTypeInt)
        OnMessage(0x1721, "_sizingOutputPan")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputTilt", UInt, 0x1722, UInt, oscTypeInt)
        OnMessage(0x1722, "_sizingOutputTilt")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputZoom", UInt, 0x1723, UInt, oscTypeInt)
        OnMessage(0x1723, "_sizingOutputZoom")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputRotate", UInt, 0x1724, UInt, oscTypeInt)
        OnMessage(0x1724, "_sizingOutputRotate")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputWidth", UInt, 0x1725, UInt, oscTypeInt)
        OnMessage(0x1725, "_sizingOutputWidth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputHeight", UInt, 0x1726, UInt, oscTypeInt)
        OnMessage(0x1726, "_sizingOutputHeight")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputPitch", UInt, 0x1727, UInt, oscTypeInt)
        OnMessage(0x1727, "_sizingOutputPitch")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputYaw", UInt, 0x1728, UInt, oscTypeInt)
        OnMessage(0x1728, "_sizingOutputYaw")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputFlip", UInt, 0x1729, UInt, oscTypeInt)
        OnMessage(0x1729, "_sizingOutputFlip")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputBlankingTop", UInt, 0x1730, UInt, oscTypeInt)
        OnMessage(0x1730, "_sizingOutputBlankingTop")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputBlankingRight", UInt, 0x1731, UInt, oscTypeInt)
        OnMessage(0x1731, "_sizingOutputBlankingRight")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputBlankingBottom", UInt, 0x1732, UInt, oscTypeInt)
        OnMessage(0x1732, "_sizingOutputBlankingBottom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputBlankingLeft", UInt, 0x1733, UInt, oscTypeInt)
        OnMessage(0x1733, "_sizingOutputBlankingLeft")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputSmooth", UInt, 0x1734, UInt, oscTypeInt)
        OnMessage(0x1734, "_sizingOutputSmooth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputResetSizing", UInt, 0x1735, UInt, oscTypeInt)
        OnMessage(0x1735, "_sizingOutputResetSizing")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingOutputResetBlanking", UInt, 0x1736, UInt, oscTypeInt)
        OnMessage(0x1736, "_sizingOutputResetBlanking")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodePan", UInt, 0x1737, UInt, oscTypeInt)
        OnMessage(0x1737, "_sizingNodePan")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeTilt", UInt, 0x1738, UInt, oscTypeInt)
        OnMessage(0x1738, "_sizingNodeTilt")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeZoom", UInt, 0x1739, UInt, oscTypeInt)
        OnMessage(0x1739, "_sizingNodeZoom")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeRotate", UInt, 0x1740, UInt, oscTypeInt)
        OnMessage(0x1740, "_sizingNodeRotate")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeKeyLock", UInt, 0x1741, UInt, oscTypeInt)
        OnMessage(0x1741, "_sizingNodeKeyLock")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeWidth", UInt, 0x1742, UInt, oscTypeInt)
        OnMessage(0x1742, "_sizingNodeWidth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeHeight", UInt, 0x1743, UInt, oscTypeInt)
        OnMessage(0x1743, "_sizingNodeHeight")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodePitch", UInt, 0x1744, UInt, oscTypeInt)
        OnMessage(0x1744, "_sizingNodePitch")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeYaw", UInt, 0x1745, UInt, oscTypeInt)
        OnMessage(0x1745, "_sizingNodeYaw")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeFlip", UInt, 0x1746, UInt, oscTypeInt)
        OnMessage(0x1746, "_sizingNodeFlip")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeBlankingTop", UInt, 0x1747, UInt, oscTypeInt)
        OnMessage(0x1747, "_sizingNodeBlankingTop")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeBlankingRight", UInt, 0x1748, UInt, oscTypeInt)
        OnMessage(0x1748, "_sizingNodeBlankingRight")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeBlankingBottom", UInt, 0x1749, UInt, oscTypeInt)
        OnMessage(0x1749, "_sizingNodeBlankingBottom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeBlankingLeft", UInt, 0x1750, UInt, oscTypeInt)
        OnMessage(0x1750, "_sizingNodeBlankingLeft")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeSmooth", UInt, 0x1751, UInt, oscTypeInt)
        OnMessage(0x1751, "_sizingNodeSmooth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeResetSizing", UInt, 0x1752, UInt, oscTypeInt)
        OnMessage(0x1752, "_sizingNodeResetSizing")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingNodeResetBlanking", UInt, 0x1753, UInt, oscTypeInt)
        OnMessage(0x1753, "_sizingNodeResetBlanking")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferencePan", UInt, 0x1754, UInt, oscTypeInt)
        OnMessage(0x1754, "_sizingReferencePan")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceTilt", UInt, 0x1755, UInt, oscTypeInt)
        OnMessage(0x1755, "_sizingReferenceTilt")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceZoom", UInt, 0x1756, UInt, oscTypeInt)
        OnMessage(0x1756, "_sizingReferenceZoom")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceRotate", UInt, 0x1757, UInt, oscTypeInt)
        OnMessage(0x1757, "_sizingReferenceRotate")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceWidth", UInt, 0x1758, UInt, oscTypeInt)
        OnMessage(0x1758, "_sizingReferenceWidth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceHeight", UInt, 0x1759, UInt, oscTypeInt)
        OnMessage(0x1759, "_sizingReferenceHeight")

        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferencePitch", UInt, 0x1760, UInt, oscTypeInt)
        OnMessage(0x1760, "_sizingReferencePitch")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceYaw", UInt, 0x1761, UInt, oscTypeInt)
        OnMessage(0x1761, "_sizingReferenceYaw")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceFlip", UInt, 0x1762, UInt, oscTypeInt)
        OnMessage(0x1762, "_sizingReferenceFlip")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceBlankingTop", UInt, 0x1763, UInt, oscTypeInt)
        OnMessage(0x1763, "_sizingReferenceBlankingTop")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceBlankingRight", UInt, 0x1764, UInt, oscTypeInt)
        OnMessage(0x1764, "_sizingReferenceBlankingRight")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceBlankingBottom", UInt, 0x1765, UInt, oscTypeInt)
        OnMessage(0x1765, "_sizingReferenceBlankingBottom")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceBlankingLeft", UInt, 0x1766, UInt, oscTypeInt)
        OnMessage(0x1766, "_sizingReferenceBlankingLeft")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceSmooth", UInt, 0x1767, UInt, oscTypeInt)
        OnMessage(0x1767, "_sizingReferenceSmooth")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceResetSizing", UInt, 0x1768, UInt, oscTypeInt)
        OnMessage(0x1768, "_sizingReferenceResetSizing")
        
        DllCall("OSC2AHK.dll\addListener", AStr, "/sizingReferenceResetBlanking", UInt, 0x1769, UInt, oscTypeInt)
        OnMessage(0x1769, "_sizingReferenceResetBlanking")
    }Else{
        ;All Panels Listeners
...
;===== HOW TO SEND MESSAGES TO TANGENT HUB AND Tangent Booster TROUGH OSC2AHK =====
...
;===== END OF OSC SENDING MESSAGES TO TANGENT HUB AND Tangent Booster =====
...
_tracking(oscType, data, msgID, hwnd){
    ...
    DllCall("OSC2AHK.dll\sendOscMessageString", AStr, OSC_SEND_IP, UInt, OSC_SEND_PORT, AStr, "/HubCommand", AStr, "ModeValue[19]")
}
...
_sizingModeEdit(oscType, data, msgID, hwnd){
    ...
    DllCall("OSC2AHK.dll\sendOscMessageString", AStr, OSC_SEND_IP, UInt, OSC_SEND_PORT, AStr, "/HubCommand", AStr, "ModeValue[20]")
}
...
_sizingModeInput(oscType, data, msgID, hwnd){
    ...
    DllCall("OSC2AHK.dll\sendOscMessageString", AStr, OSC_SEND_IP, UInt, OSC_SEND_PORT, AStr, "/HubCommand", AStr, "ModeValue[21]")
}
...
_sizingModeOutput(oscType, data, msgID, hwnd){
    ...
    DllCall("OSC2AHK.dll\sendOscMessageString", AStr, OSC_SEND_IP, UInt, OSC_SEND_PORT, AStr, "/HubCommand", AStr, "ModeValue[22]")
}
...
_sizingModeNode(oscType, data, msgID, hwnd){
    ...
    DllCall("OSC2AHK.dll\sendOscMessageString", AStr, OSC_SEND_IP, UInt, OSC_SEND_PORT, AStr, "/HubCommand", AStr, "ModeValue[23]")
}
...
_sizingModeReference(oscType, data, msgID, hwnd){
    ...
    DllCall("OSC2AHK.dll\sendOscMessageString", AStr, OSC_SEND_IP, UInt, OSC_SEND_PORT, AStr, "/HubCommand", AStr, "ModeValue[24]")
}
...
_sizingReferenceWidth(oscType, data, msgID, hwnd){
    if (data == 0){
        ResetForSpecificValue("pos_sizing_reference_widht", 1)
        Return
    }Else {
        MoveMouseAndDrag(data, "x", "pos_sizing_reference_widht")
        Return
    }
}
...
_sizingNodeWidth(oscType, data, msgID, hwnd){
    if (data == 0){
        ResetForSpecificValue("pos_sizing_node_widht", 1)
        Return
    }Else {
        MoveMouseAndDrag(data, "x", "pos_sizing_node_widht")
        Return
    }
}
...
_sizingOutputWidth(oscType, data, msgID, hwnd){
    if (data == 0){
        ResetForSpecificValue("pos_sizing_output_widht", 1)
        Return
    }Else {
        MoveMouseAndDrag(data, "x", "pos_sizing_output_widht")
        Return
    }
}
...
_sizingInputWidth(oscType, data, msgID, hwnd){
    if (data == 0){
        ResetForSpecificValue("pos_sizing_input_widht", 1)
        Return
    }Else {
        MoveMouseAndDrag(data, "x", "pos_sizing_input_widht")
        Return
    }
}
...
_start_stop_bt(){
    if (_appRunning){
        MsgBox Tangent Booster is paused
        ...
        MsgBox Tangent Booster is running
