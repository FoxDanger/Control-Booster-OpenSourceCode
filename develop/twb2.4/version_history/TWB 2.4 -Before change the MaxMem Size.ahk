...
;TODO - Create a checkbox to know if scopes are on a second screen, if yes, don't click on scopes button when changing scopes
...
;    _positionsArray3 := []
    ...
    Loop, read, %A_ScriptDir%\res_dpi_scale.ini
    {
        Loop, parse, A_LoopReadLine, %A_Tab%
        {
            ...
            switch (counter){
                Case 1:
                    PopulateArray(arrayPos, actualArray, "var", SubStr(A_Loopfield, startingPos + 1), True)
                    
                    counter++
                continue
                Case 2:
                    PopulateArray(arrayPos, actualArray, "x", SubStr(A_Loopfield, startingPos + 1))

                    counter++
                continue
                Case 3:
                    PopulateArray(arrayPos, actualArray, "y", SubStr(A_Loopfield, startingPos + 1))
                    
                    counter := 1
                    arrayPos++
                    ;Switch (actualArray){
                        ;Case 1:
                            ;arraySize := _positionsArray1.Length()
                            ;
                            ;if (arraySize >= 238){
                                ;actualArray := 2
                                ;arrayPos := 1
                            ...
                            ;}
                        ;Case 2:
                            ;arraySize := _positionsArray2.Length()
                            ;
                            ;if (arraySize >= 238){
                                ;actualArray := 2
                                ;arrayPos := 1
                            ...
                            ;}
                        ;Case 3:
                            ;arraySize := _positionsArray3.Length()
                            ;
                            ;if (arraySize >= 238){
                                ;actualArray := 3
                                ;arrayPos := 1
    ...
    vCount := NumGet(&_positionsArray1 + 4*A_PtrSize)
    MsgBox %vCount%
    
    var := GetObjectOnPositionsArray("pos_custom_dots_menu").x
    
    MsgBox %var%
    
    var2 := GetObjectOnPositionsArray("pos_hue_curves_input_hue_satxlum").x
    MsgBox %var2%
    ;DisplayArray("Positions Array 2", _positionsArray2)
    ;DisplayArray("Positions Array 3", _positionsArray3)
}

;Populate each _positionArray with the variables from res_dpi_scale.ini
PopulateArray(arrayPos, actualArray, variable, content, createNewObject := False){
    if (createNewObject){
        object := []
    ...
    Switch (actualArray){
        Case 1:
            Switch (variable){
                Case "var":
                    _positionsArray1[arrayPos].var := content
                Case "x":
                    _positionsArray1[arrayPos].x := content
                Case "y":
                    _positionsArray1[arrayPos].y := content
            }
        Case 2:
            Switch (variable){
                Case "var":
                    _positionsArray2[arrayPos].var := content
                Case "x":
                    _positionsArray2[arrayPos].x := content
                Case "y":
                    _positionsArray2[arrayPos].y := content
            }
        Case 3:
            Switch (variable){
                Case "var":
                    _positionsArray3[arrayPos].var := content
                Case "x":
                    _positionsArray3[arrayPos].x := content
                Case "y":
                    _positionsArray3[arrayPos].y := content
            }
    }
}
...
;Write in the INI file all the positions stored on the _positionsArray(x) under the respective resolution
UpdateIniResScaleFile(resolutionScale) {
    For arrayNum, array in _positionsArray1
        For index, varxy in ["var", "x", "y"]
        IniWrite, % array[varxy], %A_ScriptDir%\res_dpi_scale.ini, %resolutionScale%, %_resolutionAndScale% - %arrayNum%%varxy%
        
    For arrayNum, array in _positionsArray2
        For index, varxy in ["var", "x", "y"]
        IniWrite, % array[varxy], %A_ScriptDir%\res_dpi_scale.ini, %resolutionScale%, %_resolutionAndScale% - %arrayNum%%varxy%
        
    For arrayNum, array in _positionsArray3
        For index, varxy in ["var", "x", "y"]
        IniWrite, % array[varxy], %A_ScriptDir%\res_dpi_scale.ini, %resolutionScale%, %_resolutionAndScale% - %arrayNum%%varxy%
}
...
;Change the combobox for the latest resolution/scale saved in the _resolutionAndScale variable and write it on the INI file. After that, load all the positions inside the _positionsArray(x)
